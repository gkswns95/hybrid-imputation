            ...
            while len(obs_list_count) < seq_len:
                ...
                if min_gap < gap and gap <= max_gap:
                    ...
                    if self.params["stochastic"]:
                        ...
                        imputations_ = reshape_tensor(imputations, rescale=False, n_features=n_features, dataset=dataset).flatten(2,3)
                    ...
                    imputations_ = reshape_tensor(imputations, rescale=True, n_features=n_features, dataset=dataset).detach().cpu() # [bs, n_imp, total_players, 2]
                    gt_data_ = reshape_tensor(gt_data, rescale=True, n_features=n_features, dataset=dataset).detach().cpu()
                    mask_ = reshape_tensor(mask, rescale=False, n_features=n_features, dataset=dataset).detach().cpu() # [bs, time, total_players, 2]
