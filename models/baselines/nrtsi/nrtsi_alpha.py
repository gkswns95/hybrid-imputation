...
from models.baselines.nrtsi.nrtsi_utils import *
        ...
        if mode == "train":
            for p in range(total_players):
                ...
                while len(obs_list_count) < seq_len:
                    ...
                    if min_gap < gap and gap <= max_gap:
                        ...
                        if dataset == "football":
                            gt_data_ = reshape_tensor(gt_data, rescale=False, n_features=n_features, dataset=dataset).flatten(2,3)
                            loss += nll_gauss(gt_data_, imputations)
                        else:
                            loss += torch.mean(torch.abs(imputations - gt_data))
                            imputations_ = reshape_tensor(imputations, rescale=True, n_features=n_features, dataset=dataset).detach().cpu() # [bs, n_imp, 1, 2]
                            gt_data_ = reshape_tensor(gt_data, rescale=True, n_features=n_features, dataset=dataset).detach().cpu()
                            mask_ = reshape_tensor(p_mask_, rescale=False, n_features=n_features, dataset=dataset).detach().cpu() # [bs, time, 1, 2]
            ...
            for p in range(total_players):
                ...
                while len(obs_list_count) < seq_len:
                    ...
                    if dataset == "football":
                        gt_data = reshape_tensor(gt_data, rescale=False, n_features=n_features, dataset=dataset).flatten(2,3)
            ...
            pred_ = reshape_tensor(pred, rescale=True, n_features=n_features, dataset=dataset) # [bs, time, total_players * 2]
            ...
            if self.params["xy_sort"]:
                ...
                ret["target"] = xy_sort_tensor_v2(target, sort_indices, n_featrues=n_features, n_players=total_players, mode="restore")
