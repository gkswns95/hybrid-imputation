...
from models.utils import reshape_tensor, nll_gauss, sample_gauss, calc_coherence_loss
...
class dbhp_imputer(nn.Module):
    ...
    def train_hybrid_pred(self, ret):
        ...
        hybrid_weights_reshaped = hybrid_weights.reshape(self.bs, self.n_components, self.seq_len, -1) # [bs, comp, time, 3]
    ...
    def forward(self, data, device="cuda:0"):
        ...
        if self.params["fpi"]:
            self.fpi_z = self.fpi_st(self.x).unsqueeze(1).expand(-1, self.n_components, -1)  # [time * bs, n_agents, pi_z_dim]
        ...
        if self.params["train_hybrid"]:
            context_embeds = torch.cat([self.fpe_z, self.fpi_z], -1).reshape(self.seq_len, self.bs * self.n_components, -1)
            self.hybrid_rnn_context = context_embeds.reshape(self.seq_len, self.bs, self.n_components, -1).transpose(0,1) # [time, bs * comp, -1]
        ...
        if self.n_features == 2:
            ...
            if self.params["physics_loss"]:
                ...
                ret["physics_f_pred"] = derivative_based_pred(ret, physics_mode=mode, fb="f", dataset=self.dataset) # [bs, n_agents, time, 2] (STRNN-DAP-F)
                ret["physics_b_pred"] = derivative_based_pred(ret, physics_mode=mode, fb="b", dataset=self.dataset) # (STRNN-DAP-B)

                ret["physics_f_loss"] = torch.abs(ret["xy_target"] - ret["physics_f_pred"]).sum() / (1-ret["xy_mask"]).sum()
                ret["physics_b_loss"] = torch.abs(ret["xy_target"] - ret["physics_b_pred"]).sum() / (1-ret["xy_mask"]).sum()
                ...
                if self.params["train_hybrid"]: 
                    ...
                    ret["train_hybrid_loss"] = torch.abs((ret["train_hybrid_pred"] - ret["xy_target"]) * (1 - ret["xy_mask"])).sum() / (1-ret["xy_mask"]).sum()
                ...
                ret["train_hybrid_weights"] = ret["train_hybrid_weights"].transpose(1, 2).flatten(2, 3) # [bs, time, comp*3]
