...
from models.utils import get_dataset_config, reshape_tensor
...
def derivative_based_pred(ret, physics_mode="vel", fb="f", dataset="soccer"):
    ...
    m = ret[f"xy_mask"].flatten(0, 1)  # [bs * n_agents, time, 2]
    pxy_d = ret[f"xy_pred"].flatten(0, 1) * scale_tensor
    vxy_d = ret[f"vel_pred"].flatten(0, 1)
    ...
    if physics_mode == "vel":
        ...
        axy_d = ret[f"cartesian_accel_pred"].flatten(0, 1)
...
def calc_static_hybrid_pred2(ret):
    '''
    2 preds
    '''
...
def calc_static_hybrid_pred(ret):
    '''
    3 preds
    '''
