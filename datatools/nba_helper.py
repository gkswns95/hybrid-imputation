...
class NBADataHelper(TraceHelper):
    ...
    def calc_single_player_running_features(self, p: str, episode_traces: pd.DataFrame, remove_outliers=True, smoothing=True):        
        ...
        self.traces.loc[episode_traces.index, NBADataHelper.player_to_cols(p)] = np.stack([x, y, vx, vy, speeds, accels, ax, ay]).round(6).T
...
class NBADataAnimator():
    def __init__(
        ...
        anim_cols = [
            "traces", "target_traces", "masks", "player_circles", "ball_circle",
             "player_rectangles", "annotations", "patch_annots", "clock_info"]
    ...
    def animate_player_and_ball(
        ...
        for j in range(len(player_circles)):
            ...
            if trace_key == "main":
                if trace.shot_clock == None:
                    trace.shot_clock = 0.0

                clock_text = "Quarter {:d}\n {:02d}:{:02d}\n {:03.1f}".format(
                            int(trace.quarter),
                            int(trace.game_clock) % 3600 // 60,
                            int(trace.game_clock) % 60,
                            trace.shot_clock)
        ...
        if player_rectangles is not None:
            for i in range(len(player_rectangles)):
                if mask[f"player{i}_x"] == 0:    
                    ...
                    patch_annotations[i].set_text(NBADataAnimator.calc_trace_dist(trace[[f"player{i}_x", f"player{i}_y"]], target_trace[[f"player{i}_x", f"player{i}_y"]]))
    ...
    def plot_players_and_ball(ax : axes.Axes, trace_key : str, masks : pd.DataFrame = None):
        ...
        annotations = [ax.annotate(player_num, xy=[0, 0], color="w",
        ...
        if masks is not None and trace_key != "main":
            ...
            for _ in range(10):
                player_rectangle = Rectangle(xy=(0,0), width=1, height=1, facecolor="none", edgecolor=c, alpha=0, zorder=1, linewidth=1.5)
                patch_annot = text.Annotation(text=None, xy=(0,0), color="dimgrey", fontsize=8, zorder=2, fontweight="bold")
    ...
    def run(self):
        ...
        for trace_key in self.trace_dict.keys():
            self.anim_args.loc[trace_key] = [self.trace_dict[trace_key], self.trace_dict["main"]] + NBADataAnimator.plot_players_and_ball(ax=ax, trace_key=trace_key, masks=self.masks)
            ...
            if trace_key == "main":
                ...
                if self.show_episodes:
                    ...
                    episode_annot = ax.text(24, Constant.Y_MAX+1, episode_texts[0], fontsize=12, ha="left", va="bottom")
